# unequip (not real :P)
execute as @s[tag=red, scores={shop=0, house_color=0}] at @s run tellraw @s {"rawtext":[{"text": "§eYou cannot unequip your default color! where would your house go if you had no colors equipped?!?! :C"}]}
# equip
execute as @s[tag=shop] at @s run scoreboard players set @s house_color 0
execute as @s[tag=shop, tag=!unequip, scores={house_color=0}] at @s run playsound sfx.shop.equip @s 131 4 -27
execute as @s[tag=shop, tag=!unequip, scores={house_color=0}] at @s run tellraw @s {"rawtext":[{"text":"§aSuccessfully equipped color"}]}
